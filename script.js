...
// - TESTE DE BARRAS
function barCount(gender, barReps, barSeconds) {
  if (gender === "male") {
...
// - TESTE DE DISTANCIA E CORRIDA
function runCount(gender, runDistance, runTime) {
  if (gender === "male") {
    return (
...
function swinCount(gender, swimDistance, swimTime, diveTime) {
  if (gender === "male") {
...
function areCandidateResultsValid(
  ...
  // - ATRIBUIÇÕES
  let resultHeight = heightValid(gender, height);
  let resultTestBar = barCount(gender, barReps, barSeconds);
  let resultAbs = absValid(gender, abs);
  let resultRun = runCount(gender, runDistance, runTime);
  let resultSwim = swinCount(gender, swimDistance, swimTime, diveTime);
